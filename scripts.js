...
let WHATSAPP_NUMBER = window.WHATSAPP_NUMBER || '+573208042101';
let carrito = [];
...
function updateCartDisplay() {
    ...
    if (carrito.length === 0) {
        ...
        lista.innerHTML = carrito.map((p, i) => {
            ...
            return `
                <div class="cart-item">
                    <a href="${p.imagen || '/static/images/placeholder.jpg'}" data-fancybox="cart-gallery" data-caption="${p.nombre}">
                        <img src="${p.imagen || '/static/images/placeholder.jpg'}" alt="${p.nombre}" class="cart-item-image" style="cursor: zoom-in;">
                    </a>
...
async function enviarPedido() {
    ...
    try {
        ...
        if (data.status === 'success') {
            // Preparar mensaje para WhatsApp (Estilo Premium)
            let mensaje = `✨ *ORDEN DE COMPRA - BERAKHAH* ✨\n`;
            mensaje += `_Maison de Joaillerie_\n\n`;
            
            mensaje += `━━━━━━━━━━━━━━━━━━━━\n`;
            mensaje += `👤 *DATOS DEL CLIENTE*\n`;
            mensaje += `*Nombre:* ${nombreCliente}\n`;
            mensaje += `*Celular:* ${telefonoCliente}\n`;
            mensaje += `*Email:* ${correoCliente}\n`;
            ...
            carrito.forEach((producto) => {
                ...
                mensaje += `   Cant: ${producto.cantidad}  •  Subtotal: *${formatPrice(subtotalProducto)}*\n`;
                if (producto.imagen) {
                    mensaje += `   🔗 _Imagen:_ ${producto.imagen}\n`;
                }
                mensaje += `   ┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈\n`;
